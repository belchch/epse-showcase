...
Путь эксперта по делу: создание экспертизы → карточка дела → фотофиксация на осмотре → разметка дефектов по шаблонам с привязкой к ГОСТ → генерация отчёта.

| | |
|---|---|
| ![Создание экспертизы](docs/screenshots/01-case-create.png) | ![Карточка дела](docs/screenshots/02-case-info.png) |
| Создание экспертизы: дело, адрес, вид, организация, срок сдачи | Карточка дела: статусы, приоритет, ответственные, объект исследования |
| ![Загрузка фото](docs/screenshots/03-photo-upload.png) | ![Галерея фото](docs/screenshots/04-photo-gallery.png) |
| Пакетная загрузка фото с осмотра — напрямую в S3 по presigned URL | Галерея: фильтры по типу и локации, группировка, объединение фото |
| ![Шаблоны дефектов](docs/screenshots/05-defect-templates.png) | ![Формат отчёта](docs/screenshots/06-report-format.png) |
| Разметка дефекта: подбор шаблона по поверхности и материалу, ссылка на ГОСТ | Настройка формата отчёта перед генерацией |

![Отчёт по дефектам](docs/screenshots/07-defect-report.png)

Сгенерированный отчёт: дефекты по локациям с нормативами и фотографиями.
